...
def test_default_path():
    ...
    if actual_log_dir == expected_log_dir:
        print("[EMOJI] SUCCESS: Log directory is correct!")
    else:
        print("[EMOJI] FAILURE: Log directory mismatch!")
        return False
    ...
    if dir_perms == '755':
        print("[EMOJI] SUCCESS: Directory permissions are correct!")
    else:
        print(f"[EMOJI]  WARNING: Directory permissions are {dir_perms}, expected 755")
    ...
    if logger.log_file.exists():
        ...
        if file_perms == '666':
            print("[EMOJI] SUCCESS: File permissions are correct!")
        else:
            print(f"[EMOJI]  WARNING: File permissions are {file_perms}, expected 666")
...
def test_custom_path():
    ...
    try:
        ...
        if actual_log_dir == temp_dir:
            print("[EMOJI] SUCCESS: Custom path works correctly!")
            logger.close()
            return True
        else:
            print(f"[EMOJI] FAILURE: Expected {temp_dir}, got {actual_log_dir}")
            logger.close()
            return False
    finally:
        # Clean up
...
def main():
    ...
    for test_name, passed in results:
        status = "[EMOJI] PASSED" if passed else "[EMOJI] FAILED"
        print(f"{test_name}: {status}")
    
    all_passed = all(result[1] for result in results)
    
    if all_passed:
        print("\n[EMOJI] All tests passed!")
        return 0
    else:
        print("\n[EMOJI] Some tests failed!")
        return 1
