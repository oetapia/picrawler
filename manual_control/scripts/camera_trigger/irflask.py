...
def convert_to_mp4(h264_file, delete_original=True):
    """Convert .h264 to .mp4 and save in recordings folder"""
    ...
    with record_lock:
        if recording:
            ...
            if current_h264:
                ...
                current_h264 = None
            recording_start_time = None  # reset
...
def start_recording():
    ...
    with record_lock:
        if not recording:
            ...
            current_h264 = os.path.join(RECORDINGS_DIR, f"recording_{timestamp}.h264")
...
if __name__ == '__main__':
    camera.start()
    os.makedirs(RECORDINGS_DIR, exist_ok=True)  # ensure folder exists
