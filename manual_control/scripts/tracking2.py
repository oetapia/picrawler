...
# -*- coding: utf-8 -*-
"""
Enhanced PiCrawler Autonomous Navigation System - DUAL TOF PROTOTYPE

This is a PROTOTYPE version of tracking.py that integrates dual VL53L0X ToF sensors
via PCA9548A multiplexer for front/rear obstacle detection.

Hardware Configuration:
- PCA9548A Multiplexer at 0x70
- SD1 (Channel 1): Rear VL53L0X ToF sensor
- SD2 (Channel 2): Front VL53L0X ToF sensor
- SD7 (Channel 7): MPU6050 Accelerometer

Features:
- Front ToF sensor for forward obstacle detection
- Rear ToF sensor for backward safety checks
- Accelerometer-based tilt detection and speed control
- IR floor sensors for edge detection
- Intelligent obstacle avoidance with rear awareness
"""
...
# Multiplexer and ToF Configuration
I2C_BUS = 1
MUX_ADDRESS = 0x70
REAR_TOF_CHANNEL = 1   # SD1: Rear VL53L0X
FRONT_TOF_CHANNEL = 2  # SD2: Front VL53L0X
ACCEL_CHANNEL = 7      # SD7: MPU6050

# Legacy distance sensor configuration (DISABLED - using dual ToF instead)
DISTANCE_SENSOR_TYPE = None  # Set to None when using dual ToF
...
class EnhancedPiCrawler:
    ...
    def __init__(self, distance_sensor_type=DISTANCE_SENSOR_TYPE):
        ...
        # Initialize multiplexer and dual ToF sensors
        self.mux = None
        self.front_tof = None
        self.rear_tof = None
        ...
        if HAS_VL53L0X:
            try:
                safe_print("Initializing multiplexer and ToF sensors...")
                
                # Initialize multiplexer
                self.mux = PCA9548A(bus_number=I2C_BUS, address=MUX_ADDRESS)
                safe_print(f"{ICONS['check']} Multiplexer initialized at 0x{MUX_ADDRESS:02X}")
                
                # Initialize front ToF (Channel 2)
                safe_print(f"  Initializing front ToF on channel {FRONT_TOF_CHANNEL}...")
                self.mux.select_channel(FRONT_TOF_CHANNEL)
                time.sleep(0.05)
                self.front_tof = VL53L0X.VL53L0X(i2c_bus=I2C_BUS, i2c_address=0x29)
                self.front_tof.open()
                self.front_tof.start_ranging(VL53L0X.Vl53l0xAccuracyMode.BETTER)
                safe_print(f"{ICONS['check']} Front ToF initialized")
                
                # Initialize rear ToF (Channel 1)
                safe_print(f"  Initializing rear ToF on channel {REAR_TOF_CHANNEL}...")
                self.mux.select_channel(REAR_TOF_CHANNEL)
                time.sleep(0.05)
                self.rear_tof = VL53L0X.VL53L0X(i2c_bus=I2C_BUS, i2c_address=0x29)
                self.rear_tof.open()
                self.rear_tof.start_ranging(VL53L0X.Vl53l0xAccuracyMode.BETTER)
                safe_print(f"{ICONS['check']} Rear ToF initialized")
                
                self.tof_available = True
                
            except Exception as e:
                safe_print(f"{ICONS['warning']} ToF sensor initialization failed: {e}")
                self.tof_available = False
        else:
            safe_print(f"{ICONS['warning']} VL53L0X library not available - ToF sensors disabled")
        
        # Legacy distance sensor (disabled when using dual ToF)
        self.distance_sensor = None
        
        # Initialize accelerometer
        self.accel_available = False
        if HAS_VL53L0X and self.mux:
            try:
                safe_print(f"Initializing accelerometer on channel {ACCEL_CHANNEL}...")
                self.mux.select_channel(ACCEL_CHANNEL)
                time.sleep(0.05)
                accelerometer.wake()
                time.sleep(0.1)
            ...
            safe_print(f"{ICONS['warning']} Accelerometer initialization skipped (no multiplexer)")
    ...
    def get_distance(self):
        """Get front distance reading from ToF sensor"""
        if not self.tof_available or self.front_tof is None:
            return 999
        ...
        try:
            ...
            safe_print(f"Front ToF error: {e}")
            return 999
    
    def get_rear_distance(self):
        """Get rear distance reading from ToF sensor"""
        if not self.tof_available or self.rear_tof is None:
            return 999
        ...
        try:
            ...
            safe_print(f"Rear ToF error: {e}")
            return 999
    
    def get_tilt(self):
        """Get pitch and roll from accelerometer via multiplexer"""
        if not self.accel_available:
            return 0.0, 0.0
        
        try:
            # Switch to accelerometer channel
            if self.mux:
                self.mux.select_channel(ACCEL_CHANNEL)
                time.sleep(0.002)  # Small delay for channel switch
            ...
            safe_print(f"Accelerometer error: {e}")
            return 0.0, 0.0
    ...
    def stop(self):
        ...
        if self.front_tof:
            try:
                ...
                safe_print(f"{ICONS['check']} Front ToF closed")
            except Exception as e:
                safe_print(f"Front ToF cleanup error: {e}")
        ...
        if self.rear_tof:
            try:
                ...
                self.rear_tof.close()
                safe_print(f"{ICONS['check']} Rear ToF closed")
            except Exception as e:
                safe_print(f"Rear ToF cleanup error: {e}")
        
        # Close multiplexer
        if self.mux:
            try:
                self.mux.close()
                safe_print(f"{ICONS['check']} Multiplexer closed")
            except Exception as e:
                safe_print(f"Multiplexer cleanup error: {e}")
...
def main():
    """Main entry point"""
    safe_print(f"\n{ICONS['robot']} Enhanced PiCrawler - DUAL TOF PROTOTYPE")
    safe_print("=" * 60)
    safe_print(f"Hardware Configuration:")
    safe_print(f"  - PCA9548A Multiplexer at 0x{MUX_ADDRESS:02X}")
    safe_print(f"  - SD{REAR_TOF_CHANNEL}: Rear VL53L0X ToF sensor")
    safe_print(f"  - SD{FRONT_TOF_CHANNEL}: Front VL53L0X ToF sensor")
    safe_print(f"  - SD{ACCEL_CHANNEL}: MPU6050 Accelerometer")
    safe_print(f"Sensor fusion with multiplexed I2C sensors")
    safe_print("=" * 60 + "\n")
