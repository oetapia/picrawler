...
# Check if the robot hat is present
robot_hat_present = True  # Set to False if not using the robot hat

# Try to initialize the OLED display if the robot hat is present
if robot_hat_present:
    try:
...
if robot_hat_present:
    # If robot hat is present, run this block of code
    if oled_present:
    ...
    # If robot hat is not present, run this block of code
    print("Robot hat not detected. Running in default mode.")
    # Additional code for default mode can be placed here
    # For example, display a message on the console
    if oled_present:
        display.fill(0)
        display.text("Robot hat not detected.", 0, 0, 1)
        display.show()
    sleep(1)  # Optional delay for demonstration
